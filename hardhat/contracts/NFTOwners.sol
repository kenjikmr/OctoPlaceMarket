...
contract NFTOwners {
    /// @param tokenAddress nft smart contract address
    function getUniqueHolders(
        address tokenAddress
    ) public view returns (address[] memory) {
        ITNT721 token = ITNT721(tokenAddress);

        // Build list of unique addresses
        address[] memory uniqueHolders = new address[](token.totalSupply());
        uint uniqueCount = 0;

        for (uint i = 0; i < token.totalSupply(); i++) {
            address owner = token.ownerOf(i);

            if (isNew(owner, uniqueHolders, uniqueCount)) {
                uniqueHolders[uniqueCount] = owner;
                uniqueCount++;
            }
        }
