...
contract OctoplaceMarket is ReentrancyGuard {
    ...
    * @notice Change the fee address
    ...
    public nonReentrant {
        ...
        (bool isOwnerSupported, ) = nftContract.call(abi.encodeWithSignature("owner()"));
        if(isOwnerSupported){
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        if (creator != address(0x0)) {
            ...
            if(checkRoyalties(addressNFT)){
                (address creatorAddr, uint256 royalityAmt) = IERC2981(addressNFT).royaltyInfo(tokenId, msg.value);
                (bool success,) = payable(creatorAddr).call{value : royalityAmt}("");
    ...
    */
    ...
    // set creator fee
    function setCreatorFeeBasisPointsByCreator(uint256 feeBasisPoints, address creatorAddress, address NFTAddress)
    public {
        require(feeBasisPoints <= 1000, "Sales Fee cant be higher than 10%");
